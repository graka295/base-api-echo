...
func SwaggerInfo() {
	...
	docs.SwaggerInfo.Title = GetConfiguration("server.name")
	docs.SwaggerInfo.Description = fmt.Sprintf("This is a %s server.", GetConfiguration("server.name"))
	docs.SwaggerInfo.Version = GetConfiguration("server.version")

	docs.SwaggerInfo.Host = GetConfiguration("swagger.url")
	docs.SwaggerInfo.Schemes = []string{GetConfiguration("swagger.schemes")}
}

// ConfigSwagger stores echoSwagger configuration variables.
...
func URL(url string) func(c *ConfigSwagger) {
	return func(c *ConfigSwagger) {
		c.URL = url
	}
}
...
func EchoWrapHandler(confs ...func(c *ConfigSwagger)) echo.HandlerFunc {
